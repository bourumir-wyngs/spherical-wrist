# Documentation

This documentation is written for Python users of `spherical-wrist`.

Start here:

- [Installation](installation.md)
- [Quickstart](quickstart.md)
- [Core Concepts](concepts.md)

Task guides:

- [Robot Models](robots.md)
- [Transforms And Units](transforms-and-units.md)
- [Jacobian](jacobian.md)
- [Meshes And Collisions](meshes-and-collisions.md)
- [Path Planning](path-planning.md)
- [Visualization](visualization.md)

Reference and support:

- [API Reference](api-reference.md)
- [Troubleshooting](troubleshooting.md)
- [Development With Maturin](development-maturin.md)
- [Examples](../python/examples/README.md)

The two examples most users should study first are:

- [cartesian_stroke.py](../python/examples/cartesian_stroke.py)
- [path_planning_rrt.py](../python/examples/path_planning_rrt.py)
